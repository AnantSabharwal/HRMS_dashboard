...
class OverviewCalculator(BasicAnalysis):
    def __init__(self, df):
        """
        Initialize OverviewCalculator class.
        ...
        """
        self._columns_to_add = ['BD support', 'Studies/ Analysis (billable hours)', 'Publications/ conferences/ panel discussions/ white papers/webinars/ research', 'People management (hiring/ mentoring/ engagement)', 'Trainings/ Self-development/\
                                    upskilling', 'Admin', 'Time-off']
        self._rows_to_add = ['BD-Head',
                             'Ops- Head- Utilities', 'Ops Head- Economics',
                             'Ops Head- Power Sys Studies & Equipment Modelling', 'Ops- Global\
                                    Head', 'Ops-Regional Leads', 'Ops PE/ TL', 'Ops SE', 'Ops AE']
        super().__init__(df)
    ...
    def _generate_result_table(self):
        ...
        grouped_df['Percentage'] = grouped_df.apply(
            lambda row: (row['Duration'] / self._total_duration_by_name[row['Category']]) * 100, axis=1)

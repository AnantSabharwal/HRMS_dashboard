...
class ByTeamCalculator(BasicAnalysis):
    def __init__(self, df):
        ...
        rows_to_add (list): List of row names to add if missing. ['BD support', 'Studies/ Analysis (billable hours)',
        ...
        """
        self._rows_to_add = ['BD support', 'Studies/ Analysis (billable hours)',
                             'Publications/ conferences/ panel discussions/ white papers/ webinars/ research',
                             'People management (hiring/ mentoring/ engagement)',
                             'Trainings/ Self-development/ upskilling',
                             'Admin', 'Time-off']
        self._columns_to_add = []
        super().__init__(df)
        self._total_duration_by_team = None
    ...
    def get_total_duration(self):
        return self._total_duration_by_team

    def total_duration(self):
        self._total_duration_by_team = self._calculate_total_duration()

    def _group_data(self):
        self._grouped_df = self._df.groupby(['Team', 'Work Items'])['Duration'].sum().reset_index()
        return self._grouped_df

    def _calculate_total_duration(self):
        return self._df.groupby('Team')['Duration'].sum()

    def _generate_result_table(self):
        grouped_df = self._grouped_df.copy()
        grouped_df['Percentage'] = grouped_df.apply(
            lambda row: (row['Duration'] / self._total_duration_by_team[row['Team']]) * 100, axis=1)
        result_table = pd.pivot_table(grouped_df, values='Percentage', index='Team', columns='Work Items',fill_value=0)
